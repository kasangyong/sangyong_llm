...
예상 시간(V100S 32GB 1장, fp16). **실측이 아니라 추정이다.**

FLOPs/token = 6N + 6·L·T·d 로 계산했다(역전파 포함, 어텐션 로짓/값 항 포함).
실효 성능은 measured matmul peak 88.8 TFLOPS의 25~40%로 잡았다 — 작은 모델은
커널 실행 오버헤드 비중이 커서 MFU가 낮게 나온다.

| 모델 | FLOPs/token | 2.68B 토큰 | 6.6B 토큰 |
|---|---|---|---|
| 53m | 0.36 G | 9~13시간 | 22~33시간 |
| 282m | 2.00 G | 42~59시간 | **4.4~6.1일** |

첫 100스텝의 tok/s를 보고 다시 계산할 것. `probe_vram.py`가 실측 tok/s로
같은 추정을 출력한다.
...
## 현재 상태 (2026-09-18) — 본 학습 완료

- 검증 143항목 통과 (9개 스위트)
- **`282m` 프리트레이닝 완료.** iter 13,120 / val 0.8190 (ppl 2.27) /
  문법 유효율 100.0% / pass@5 60.0%. 미학습 기준선은 60.0% / 0.0%였다
- 결과물은 `checkpoints/282m_iter13120_weights.pt`(fp32 1,078MB)와
  `282m_iter13120_bf16.pt`(539MB). 둘 다 추론 전용이라 옵티마이저 상태가 없다
- **SFT와 툴 레이어는 실행되지 않았다.** 프리트레이닝이 끝난 시점에 서버
  자원이 회수됐다. 282M 학습은 4.52GB가 필요해 6GB 노트북에 안 들어간다.
  코드·데이터셋·테스트는 완성돼 있다
- GPU가 다시 생기면 `finetune/sft.py --base <추론용 파일>`로 바로 이어붙일
  수 있다. SFT는 옵티마이저 상태를 이어받지 않으므로 추론 전용 파일로도 된다

### 다음에 서버를 다시 쓰게 되면

`data/prepare.py tokenize`는 재개가 안 된다(출력을 `wb`로 연다). 중간에
끊기면 처음부터 다시 쓰므로 `tmux` 안에서 끝까지 돌릴 것. 참고로 로컬
토큰화도 이 이유로 2,677,510,268 토큰 지점에서 버려진 적이 있다.
