...
### 프롬프트를 영어로 바꿨다 (2026-09-18)

위 수치는 전부 **한국어 독스트링 프롬프트**로 잰 것이다. 이게 측정을
왜곡한다는 것을 나중에 확인하고 `eval/problems.py`의 프롬프트를 영어로
바꿨다.

모델은 codeparrot-clean(영어/코드)만 봤고, 이 토크나이저는 파이썬 코드로
학습해서 한글 조각을 거의 병합하지 않았다. 같은 뜻을 적는 데 드는 토큰이
4~7배다.

| 독스트링 | 토큰 |
|---|---|
| `두 수를 더해 반환한다.` | 31 |
| `Return the sum of two numbers.` | 7 |
| `문자열을 뒤집어 반환한다.` | 36 |
| `Return the string reversed.` | 5 |

토큰만 먹는 게 아니라 학습 분포에 없는 문자열이라 모델이 이어 쓸 단서도
잃는다. 같은 체크포인트(iter 13,120)를 프롬프트만 바꿔 다시 채점했다.

| 프롬프트 | 문법 유효율 | pass@5 | 샘플 단위 |
|---|---|---|---|
| 한국어 | 98.0% | 40.0% | 24.0% (12/50) |
| 영어 | 100.0% | **60.0%** | 36.0% (18/50) |

영어로 세 번 돌리면 pass@5 60% / 60% / 70%, 샘플 단위 34% / 36% / 40%가
나온다. 한국어 쪽과는 계속 벌어진다. 다만 위 한국어 40%는 이 저장소의
다른 측정(50% / 28%)과도 10%p 차이가 나는데, 그게 바로 다음 절에서 말하는
표본 잡음이다. 토큰 비용 쪽은 잡음이 없는 결정적 근거다.

**앞 절의 표와 격자 그림은 전환 이전 측정이라 그대로 뒀다.** 비교 대상인
`best.pt`(iter 13,000)가 로컬에 없어 같은 조건으로 다시 잴 수 없기 때문이다.
지금 `eval/harness.py`를 돌리면 영어 프롬프트 기준 수치가 나온다.
...
| `test_export.py` | 5 | bf16 변환이 가중치 공유를 끊는지, 내보내기 왕복 일치 |
